...
public class DeliveryPermissionChecker {
    ...
    // 배송 단건 조회 — 권한 없으면 403
    public void checkDeliveryReadPermission(DeliveryEntity delivery, UUID userId, String role,
        ...
        if (COMPANY_MANAGER.equals(role)) return;
        throw new BusinessException(CommonErrorCode.FORBIDDEN);
    }

    // 배송 수정 / 상태변경 — MASTER, HUB_MANAGER(자기 허브)만 허용
    // DELIVERY_MANAGER는 상태변경만 허용 (별도 메서드로 분리)
    public void checkDeliveryWritePermission(DeliveryEntity delivery, UUID userId, String role, UUID hubId) {
        if (MASTER.equals(role)) return;
        if (HUB_MANAGER.equals(role)) {
            if (hubId != null && (hubId.equals(delivery.getSourceHubId()) || hubId.equals(delivery.getDestinationHubId()))) return;
            throw new BusinessException(CommonErrorCode.FORBIDDEN);
    ...
    public void checkDeliveryStatusChangePermission(DeliveryEntity delivery, UUID userId, String role, UUID hubId) {
        if (MASTER.equals(role)) return;
        if (HUB_MANAGER.equals(role)) {
            if (hubId != null && (hubId.equals(delivery.getSourceHubId()) || hubId.equals(delivery.getDestinationHubId()))) return;
            throw new BusinessException(CommonErrorCode.FORBIDDEN);
    ...
    // 배송 삭제 — MASTER만 허용
    public void checkDeletePermission(String role) {
    ...
    public void checkManagerReadPermission(DeliveryManagerEntity manager, UUID userId, String role, UUID hubId) {
        if (MASTER.equals(role)) return;
        if (HUB_MANAGER.equals(role)) {
    ...
    public void checkManagerWritePermission(UUID targetHubId, UUID userId, String role, UUID hubId) {
        if (MASTER.equals(role)) return;
        if (HUB_MANAGER.equals(role)) {
    ...
    public void checkManagerSelfWritePermission(DeliveryManagerEntity manager, UUID userId, String role, UUID hubId) {
        if (MASTER.equals(role)) return;
        if (HUB_MANAGER.equals(role)) {
            if (hubId != null && hubId.equals(manager.getHubId())) return;
            throw new BusinessException(CommonErrorCode.FORBIDDEN);
